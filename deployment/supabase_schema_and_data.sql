-- =====================================================
-- SCHEMA & DATA NEXTORI - SUPABASE (POSTGRESQL)
-- Source: nextori_db_v2 (21).sql (dernier export)
-- Encodage: 100% UTF-8 natif (accents impeccables)
...
DROP TABLE IF EXISTS attente_fonctionnalite CASCADE;
DROP TABLE IF EXISTS badge CASCADE;
DROP TABLE IF EXISTS badge_utilisateur CASCADE;
DROP TABLE IF EXISTS connexion_utilisateur CASCADE;
DROP TABLE IF EXISTS favori CASCADE;
DROP TABLE IF EXISTS filiere CASCADE;
DROP TABLE IF EXISTS filiere_critere CASCADE;
DROP TABLE IF EXISTS hesitation_critere CASCADE;
DROP TABLE IF EXISTS hesitation_option CASCADE;
DROP TABLE IF EXISTS hesitation_question CASCADE;
DROP TABLE IF EXISTS hesitation_reponse CASCADE;
DROP TABLE IF EXISTS hesitation_reponse_utilisateur CASCADE;
DROP TABLE IF EXISTS hesitation_test CASCADE;
DROP TABLE IF EXISTS historique CASCADE;
DROP TABLE IF EXISTS metier CASCADE;
DROP TABLE IF EXISTS metier_critere CASCADE;
DROP TABLE IF EXISTS metier_filiere CASCADE;
DROP TABLE IF EXISTS metier_serie CASCADE;
...
DROP TABLE IF EXISTS test_riasec CASCADE;
DROP TABLE IF EXISTS universite CASCADE;
DROP TABLE IF EXISTS universite_detail CASCADE;
DROP TABLE IF EXISTS universite_filiere CASCADE;
...
CREATE TABLE IF NOT EXISTS attente_fonctionnalite (
id_attente SERIAL,
id_user INTEGER  NOT NULL,
fonctionnalite VARCHAR(100)  NOT NULL,
date_inscription TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
statut VARCHAR(50)  DEFAULT 'en_attente',
date_notification TIMESTAMP DEFAULT NULL,
...
id_badge_utilisateur SERIAL,
id_user INTEGER  NOT NULL,
id_badge INTEGER  NOT NULL,
date_obtention DATE  NOT NULL,
    PRIMARY KEY (id_badge_utilisateur)
);
...
CREATE TABLE IF NOT EXISTS filiere_critere (
id_filiere_critere SERIAL,
id_filiere INTEGER  NOT NULL,
id_critere INTEGER  NOT NULL,
valeur DECIMAL(4,2)  NOT NULL DEFAULT 0.00,
    PRIMARY KEY (id_filiere_critere)
);

CREATE TABLE IF NOT EXISTS hesitation_critere (
id_critere SERIAL,
code VARCHAR(50)  NOT NULL,
nom VARCHAR(150)  NOT NULL,
...
CREATE TABLE IF NOT EXISTS hesitation_option (
id_hesitation_option SERIAL,
id_hesitation_test INTEGER  NOT NULL,
id_metier INTEGER  DEFAULT NULL,
id_filiere INTEGER  DEFAULT NULL,
ordre INTEGER  NOT NULL,
    PRIMARY KEY (id_hesitation_option)
);

CREATE TABLE IF NOT EXISTS hesitation_question (
id_question SERIAL,
...
CREATE TABLE IF NOT EXISTS hesitation_reponse (
id_reponse SERIAL,
...
CREATE TABLE IF NOT EXISTS hesitation_reponse_utilisateur (
id_reponse_utilisateur SERIAL,
id_hesitation_test INTEGER  NOT NULL,
id_question INTEGER  NOT NULL,
...
CREATE TABLE IF NOT EXISTS hesitation_test (
id_hesitation_test SERIAL,
id_user INTEGER  NOT NULL,
type_choix VARCHAR(50)  NOT NULL,
date_creation TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
statut VARCHAR(50)  NOT NULL DEFAULT 'en_cours',
    PRIMARY KEY (id_hesitation_test)
);
...
accessible_test BOOLEAN  NOT NULL DEFAULT false,
    PRIMARY KEY (id_metier)
);

CREATE TABLE IF NOT EXISTS metier_critere (
id_metier_critere SERIAL,
id_metier INTEGER  NOT NULL,
id_critere INTEGER  NOT NULL,
valeur DECIMAL(4,2)  NOT NULL DEFAULT 0.00,
    PRIMARY KEY (id_metier_critere)
);

CREATE TABLE IF NOT EXISTS metier_filiere (
id_metier INTEGER  NOT NULL,
id_filiere INTEGER  NOT NULL,
    PRIMARY KEY (id_metier, id_filiere)
);

CREATE TABLE IF NOT EXISTS metier_serie (
id_metier INTEGER  NOT NULL,
id_serie INTEGER  NOT NULL,
niveau_compatibilite VARCHAR(50)  DEFAULT 'directe',
    PRIMARY KEY (id_metier, id_serie)
);
...
type_riasec VARCHAR(50)  NOT NULL,
...
CREATE TABLE IF NOT EXISTS test_riasec (
id_test SERIAL,
id_user INTEGER  NOT NULL,
id_questionnaire INTEGER  NOT NULL,
...
CREATE TABLE IF NOT EXISTS universite_detail (
id_detail SERIAL,
...
id_universite INTEGER  NOT NULL,
id_filiere INTEGER  NOT NULL,
    PRIMARY KEY (id_universite, id_filiere)
);
...
(1, 'contenu', 'Contenu du métier ou de la formation', 'attirance', true),
(2, 'sens', 'Sens et utilité', 'attirance', true),
(3, 'debouches', 'Perspectives professionnelles', 'attirance', true),
(4, 'conditions', 'Conditions de travail', 'attirance', true),
(5, 'progression', 'Possibilités de progression', 'attirance', true),
(6, 'analyse', 'Analyse', 'force', true),
(7, 'creation', 'Créativité', 'force', true),
(8, 'relation', 'Relation et accompagnement', 'force', true),
(9, 'organisation', 'Organisation', 'force', true),
(10, 'action', 'Initiative et passage à l''action', 'force', true),
(11, 'interet', 'Intérêt personnel', 'attente', true),
(12, 'capacites', 'Capacités et points forts', 'attente', true),
(13, 'stabilite', 'Stabilité et sécurité', 'attente', true);

INSERT INTO hesitation_question (id_question, question, ordre, actif) VALUES
(1, 'Quand tu penses aux options que tu hésites à choisir, qu''est-ce qui t''attire le plus ?', 1, true),
(2, 'Dans quelle situation te sens-tu généralement le plus à l''aise ?', 2, true),
(3, 'Qu''est-ce qui rend ton choix le plus difficile aujourd''hui ?', 3, true),
(4, 'Si tu devais aujourd''hui donner le plus de poids à un seul critère, lequel serait-ce ?', 4, true),
(5, 'Qu''est-ce qui t''aiderait le plus à avancer vers une décision ?', 5, true);
...
(1, 'Test RIASEC Standard', 'Questionnaire d''orientation professionnelle basé sur la théorie de John Holland et adapté au contexte africain.', '1.0', true, '2026-07-23 12:51:37');
...
INSERT INTO avis (id_avis, id_user, note, commentaire, afficher, approuve, date_creation) VALUES
...
-- =====================================================
-- 4. CONTRAINTES DE CLES ETRANGERES (FOREIGN KEYS)
-- =====================================================

ALTER TABLE attente_fonctionnalite ADD CONSTRAINT fk_attente_fonctionnalite_user FOREIGN KEY (id_user) REFERENCES utilisateur (id_user) ON DELETE CASCADE;
ALTER TABLE badge_utilisateur ADD CONSTRAINT fk_badge_utilisateur_badge FOREIGN KEY (id_badge) REFERENCES badge (id_badge) ON DELETE CASCADE ON UPDATE CASCADE;
ALTER TABLE badge_utilisateur ADD CONSTRAINT fk_badge_utilisateur_user FOREIGN KEY (id_user) REFERENCES utilisateur (id_user) ON DELETE CASCADE ON UPDATE CASCADE;
ALTER TABLE connexion_utilisateur ADD CONSTRAINT fk_connexion_utilisateur FOREIGN KEY (id_user) REFERENCES utilisateur (id_user) ON DELETE CASCADE ON UPDATE CASCADE;
ALTER TABLE favori ADD CONSTRAINT fk_favori_metier FOREIGN KEY (id_metier) REFERENCES metier (id_metier) ON DELETE CASCADE;
ALTER TABLE favori ADD CONSTRAINT fk_favori_user FOREIGN KEY (id_user) REFERENCES utilisateur (id_user) ON DELETE CASCADE;
ALTER TABLE filiere_critere ADD CONSTRAINT fk_filiere_critere_critere FOREIGN KEY (id_critere) REFERENCES hesitation_critere (id_critere) ON DELETE CASCADE ON UPDATE CASCADE;
ALTER TABLE filiere_critere ADD CONSTRAINT fk_filiere_critere_filiere FOREIGN KEY (id_filiere) REFERENCES filiere (id_filiere) ON DELETE CASCADE ON UPDATE CASCADE;
ALTER TABLE hesitation_option ADD CONSTRAINT fk_hesitation_option_filiere FOREIGN KEY (id_filiere) REFERENCES filiere (id_filiere) ON DELETE CASCADE;
ALTER TABLE hesitation_option ADD CONSTRAINT fk_hesitation_option_metier FOREIGN KEY (id_metier) REFERENCES metier (id_metier) ON DELETE CASCADE;
ALTER TABLE hesitation_option ADD CONSTRAINT fk_hesitation_option_test FOREIGN KEY (id_hesitation_test) REFERENCES hesitation_test (id_hesitation_test) ON DELETE CASCADE;
ALTER TABLE hesitation_reponse ADD CONSTRAINT fk_hesitation_reponse_question FOREIGN KEY (id_question) REFERENCES hesitation_question (id_question) ON DELETE CASCADE;
ALTER TABLE hesitation_reponse_utilisateur ADD CONSTRAINT fk_hesitation_ru_question FOREIGN KEY (id_question) REFERENCES hesitation_question (id_question) ON DELETE CASCADE;
ALTER TABLE hesitation_reponse_utilisateur ADD CONSTRAINT fk_hesitation_ru_reponse FOREIGN KEY (id_reponse) REFERENCES hesitation_reponse (id_reponse) ON DELETE CASCADE;
ALTER TABLE hesitation_reponse_utilisateur ADD CONSTRAINT fk_hesitation_ru_test FOREIGN KEY (id_hesitation_test) REFERENCES hesitation_test (id_hesitation_test) ON DELETE CASCADE;
ALTER TABLE hesitation_test ADD CONSTRAINT fk_hesitation_test_user FOREIGN KEY (id_user) REFERENCES utilisateur (id_user) ON DELETE CASCADE;
ALTER TABLE historique ADD CONSTRAINT fk_historique_user FOREIGN KEY (id_user) REFERENCES utilisateur (id_user) ON DELETE CASCADE;
ALTER TABLE metier_critere ADD CONSTRAINT fk_metier_critere_critere FOREIGN KEY (id_critere) REFERENCES hesitation_critere (id_critere) ON DELETE CASCADE ON UPDATE CASCADE;
ALTER TABLE metier_critere ADD CONSTRAINT fk_metier_critere_metier FOREIGN KEY (id_metier) REFERENCES metier (id_metier) ON DELETE CASCADE ON UPDATE CASCADE;
ALTER TABLE metier_filiere ADD CONSTRAINT fk_metier_filiere_filiere FOREIGN KEY (id_filiere) REFERENCES filiere (id_filiere) ON DELETE CASCADE;
ALTER TABLE metier_filiere ADD CONSTRAINT fk_metier_filiere_metier FOREIGN KEY (id_metier) REFERENCES metier (id_metier) ON DELETE CASCADE;
ALTER TABLE metier_serie ADD CONSTRAINT metier_serie_ibfk_1 FOREIGN KEY (id_metier) REFERENCES metier (id_metier) ;
ALTER TABLE metier_serie ADD CONSTRAINT metier_serie_ibfk_2 FOREIGN KEY (id_serie) REFERENCES serie (id_serie) ;
ALTER TABLE proposition ADD CONSTRAINT fk_proposition_question FOREIGN KEY (id_question) REFERENCES question (id_question) ON DELETE CASCADE;
ALTER TABLE question ADD CONSTRAINT fk_question_questionnaire FOREIGN KEY (id_questionnaire) REFERENCES questionnaire (id_questionnaire) ON DELETE CASCADE;
ALTER TABLE recommandation ADD CONSTRAINT fk_recommandation_metier FOREIGN KEY (id_metier) REFERENCES metier (id_metier) ON DELETE CASCADE;
ALTER TABLE recommandation ADD CONSTRAINT fk_recommandation_test FOREIGN KEY (id_test) REFERENCES test_riasec (id_test) ON DELETE CASCADE;
ALTER TABLE reponse ADD CONSTRAINT fk_reponse_proposition FOREIGN KEY (id_proposition) REFERENCES proposition (id_proposition) ON DELETE CASCADE;
ALTER TABLE reponse ADD CONSTRAINT fk_reponse_test FOREIGN KEY (id_test) REFERENCES test_riasec (id_test) ON DELETE CASCADE;
ALTER TABLE test_riasec ADD CONSTRAINT fk_test_questionnaire FOREIGN KEY (id_questionnaire) REFERENCES questionnaire (id_questionnaire) ON UPDATE CASCADE;
ALTER TABLE test_riasec ADD CONSTRAINT fk_test_user FOREIGN KEY (id_user) REFERENCES utilisateur (id_user) ON DELETE CASCADE;
ALTER TABLE universite_detail ADD CONSTRAINT universite_detail_ibfk_1 FOREIGN KEY (id_universite) REFERENCES universite (id_universite) ON DELETE CASCADE;
ALTER TABLE universite_filiere ADD CONSTRAINT fk_universite_filiere_filiere FOREIGN KEY (id_filiere) REFERENCES filiere (id_filiere) ON DELETE CASCADE;
ALTER TABLE universite_filiere ADD CONSTRAINT fk_universite_filiere_universite FOREIGN KEY (id_universite) REFERENCES universite (id_universite) ON DELETE CASCADE;
ALTER TABLE utilisateur ADD CONSTRAINT fk_utilisateur_serie FOREIGN KEY (id_serie) REFERENCES serie (id_serie) ;
ALTER TABLE avis ADD CONSTRAINT fk_avis_user FOREIGN KEY (id_user) REFERENCES utilisateur (id_user) ON DELETE CASCADE;

-- =====================================================
-- 5. INDEX
-- =====================================================

CREATE UNIQUE INDEX IF NOT EXISTS idx_attente_fonctionnalite_unique_attente_fonctionnalite ON attente_fonctionnalite (id_user, fonctionnalite);
CREATE UNIQUE INDEX IF NOT EXISTS idx_badge_code ON badge (code);
CREATE UNIQUE INDEX IF NOT EXISTS idx_badge_utilisateur_uq_badge_utilisateur ON badge_utilisateur (id_user, id_badge);
CREATE INDEX IF NOT EXISTS idx_badge_utilisateur_fk_badge_utilisateur_badge ON badge_utilisateur (id_badge);
CREATE UNIQUE INDEX IF NOT EXISTS idx_connexion_utilisateur_uq_connexion_utilisateur ON connexion_utilisateur (id_user, date_connexion);
CREATE UNIQUE INDEX IF NOT EXISTS idx_favori_uk_favori ON favori (id_user, id_metier);
CREATE INDEX IF NOT EXISTS idx_favori_fk_favori_metier ON favori (id_metier);
CREATE UNIQUE INDEX IF NOT EXISTS idx_filiere_nom ON filiere (nom);
CREATE UNIQUE INDEX IF NOT EXISTS idx_filiere_critere_uk_filiere_critere ON filiere_critere (id_filiere, id_critere);
CREATE INDEX IF NOT EXISTS idx_filiere_critere_fk_filiere_critere_critere ON filiere_critere (id_critere);
CREATE UNIQUE INDEX IF NOT EXISTS idx_hesitation_critere_uk_hesitation_critere_code ON hesitation_critere (code);
CREATE INDEX IF NOT EXISTS idx_hesitation_option_fk_hesitation_option_test ON hesitation_option (id_hesitation_test);
CREATE INDEX IF NOT EXISTS idx_hesitation_option_fk_hesitation_option_metier ON hesitation_option (id_metier);
CREATE INDEX IF NOT EXISTS idx_hesitation_option_fk_hesitation_option_filiere ON hesitation_option (id_filiere);
CREATE INDEX IF NOT EXISTS idx_hesitation_reponse_fk_hesitation_reponse_question ON hesitation_reponse (id_question);
CREATE UNIQUE INDEX IF NOT EXISTS idx_hesitation_reponse_utilisateur_id_hesitation_test ON hesitation_reponse_utilisateur (id_hesitation_test, id_question);
CREATE INDEX IF NOT EXISTS idx_hesitation_reponse_utilisateur_fk_hesitation_ru_question ON hesitation_reponse_utilisateur (id_question);
CREATE INDEX IF NOT EXISTS idx_hesitation_reponse_utilisateur_fk_hesitation_ru_reponse ON hesitation_reponse_utilisateur (id_reponse);
CREATE INDEX IF NOT EXISTS idx_hesitation_test_fk_hesitation_test_user ON hesitation_test (id_user);
CREATE INDEX IF NOT EXISTS idx_historique_fk_historique_user ON historique (id_user);
CREATE UNIQUE INDEX IF NOT EXISTS idx_metier_nom ON metier (nom);
CREATE UNIQUE INDEX IF NOT EXISTS idx_metier_critere_uk_metier_critere ON metier_critere (id_metier, id_critere);
CREATE INDEX IF NOT EXISTS idx_metier_critere_fk_metier_critere_critere ON metier_critere (id_critere);
CREATE INDEX IF NOT EXISTS idx_metier_filiere_fk_metier_filiere_filiere ON metier_filiere (id_filiere);
CREATE INDEX IF NOT EXISTS idx_metier_serie_id_serie ON metier_serie (id_serie);
CREATE INDEX IF NOT EXISTS idx_proposition_fk_proposition_question ON proposition (id_question);
CREATE INDEX IF NOT EXISTS idx_question_fk_question_questionnaire ON question (id_questionnaire);
CREATE INDEX IF NOT EXISTS idx_recommandation_fk_recommandation_test ON recommandation (id_test);
CREATE INDEX IF NOT EXISTS idx_recommandation_fk_recommandation_metier ON recommandation (id_metier);
CREATE UNIQUE INDEX IF NOT EXISTS idx_reponse_uk_reponse ON reponse (id_test, id_proposition);
CREATE INDEX IF NOT EXISTS idx_reponse_fk_reponse_proposition ON reponse (id_proposition);
CREATE UNIQUE INDEX IF NOT EXISTS idx_serie_nom ON serie (nom);
CREATE INDEX IF NOT EXISTS idx_test_riasec_fk_test_user ON test_riasec (id_user);
CREATE INDEX IF NOT EXISTS idx_test_riasec_fk_test_questionnaire ON test_riasec (id_questionnaire);
CREATE UNIQUE INDEX IF NOT EXISTS idx_universite_nom ON universite (nom);
CREATE INDEX IF NOT EXISTS idx_universite_detail_id_universite ON universite_detail (id_universite);
CREATE INDEX IF NOT EXISTS idx_universite_filiere_fk_universite_filiere_filiere ON universite_filiere (id_filiere);
CREATE UNIQUE INDEX IF NOT EXISTS idx_utilisateur_email ON utilisateur (email);
CREATE INDEX IF NOT EXISTS idx_utilisateur_fk_utilisateur_serie ON utilisateur (id_serie);

-- =====================================================
-- 6. SYNCHRONISATION DES SEQUENCES AUTO-INCREMENT
-- =====================================================

SELECT setval(pg_get_serial_sequence('attente_fonctionnalite', 'id_attente'), COALESCE(MAX(id_attente), 1)) FROM attente_fonctionnalite;
SELECT setval(pg_get_serial_sequence('badge', 'id_badge'), COALESCE(MAX(id_badge), 1)) FROM badge;
SELECT setval(pg_get_serial_sequence('badge_utilisateur', 'id_badge_utilisateur'), COALESCE(MAX(id_badge_utilisateur), 1)) FROM badge_utilisateur;
SELECT setval(pg_get_serial_sequence('connexion_utilisateur', 'id_connexion'), COALESCE(MAX(id_connexion), 1)) FROM connexion_utilisateur;
SELECT setval(pg_get_serial_sequence('favori', 'id_favori'), COALESCE(MAX(id_favori), 1)) FROM favori;
SELECT setval(pg_get_serial_sequence('filiere', 'id_filiere'), COALESCE(MAX(id_filiere), 1)) FROM filiere;
SELECT setval(pg_get_serial_sequence('filiere_critere', 'id_filiere_critere'), COALESCE(MAX(id_filiere_critere), 1)) FROM filiere_critere;
SELECT setval(pg_get_serial_sequence('hesitation_critere', 'id_critere'), COALESCE(MAX(id_critere), 1)) FROM hesitation_critere;
SELECT setval(pg_get_serial_sequence('hesitation_option', 'id_hesitation_option'), COALESCE(MAX(id_hesitation_option), 1)) FROM hesitation_option;
SELECT setval(pg_get_serial_sequence('hesitation_question', 'id_question'), COALESCE(MAX(id_question), 1)) FROM hesitation_question;
SELECT setval(pg_get_serial_sequence('hesitation_reponse', 'id_reponse'), COALESCE(MAX(id_reponse), 1)) FROM hesitation_reponse;
SELECT setval(pg_get_serial_sequence('hesitation_reponse_utilisateur', 'id_reponse_utilisateur'), COALESCE(MAX(id_reponse_utilisateur), 1)) FROM hesitation_reponse_utilisateur;
SELECT setval(pg_get_serial_sequence('hesitation_test', 'id_hesitation_test'), COALESCE(MAX(id_hesitation_test), 1)) FROM hesitation_test;
SELECT setval(pg_get_serial_sequence('historique', 'id_historique'), COALESCE(MAX(id_historique), 1)) FROM historique;
SELECT setval(pg_get_serial_sequence('metier', 'id_metier'), COALESCE(MAX(id_metier), 1)) FROM metier;
SELECT setval(pg_get_serial_sequence('metier_critere', 'id_metier_critere'), COALESCE(MAX(id_metier_critere), 1)) FROM metier_critere;
SELECT setval(pg_get_serial_sequence('proposition', 'id_proposition'), COALESCE(MAX(id_proposition), 1)) FROM proposition;
SELECT setval(pg_get_serial_sequence('question', 'id_question'), COALESCE(MAX(id_question), 1)) FROM question;
SELECT setval(pg_get_serial_sequence('questionnaire', 'id_questionnaire'), COALESCE(MAX(id_questionnaire), 1)) FROM questionnaire;
SELECT setval(pg_get_serial_sequence('recommandation', 'id_recommandation'), COALESCE(MAX(id_recommandation), 1)) FROM recommandation;
SELECT setval(pg_get_serial_sequence('reponse', 'id_reponse'), COALESCE(MAX(id_reponse), 1)) FROM reponse;
SELECT setval(pg_get_serial_sequence('serie', 'id_serie'), COALESCE(MAX(id_serie), 1)) FROM serie;
SELECT setval(pg_get_serial_sequence('test_riasec', 'id_test'), COALESCE(MAX(id_test), 1)) FROM test_riasec;
SELECT setval(pg_get_serial_sequence('universite', 'id_universite'), COALESCE(MAX(id_universite), 1)) FROM universite;
SELECT setval(pg_get_serial_sequence('universite_detail', 'id_detail'), COALESCE(MAX(id_detail), 1)) FROM universite_detail;
SELECT setval(pg_get_serial_sequence('utilisateur', 'id_user'), COALESCE(MAX(id_user), 1)) FROM utilisateur;
SELECT setval(pg_get_serial_sequence('avis', 'id_avis'), COALESCE(MAX(id_avis), 1)) FROM avis;
